...
class AvecRolloutBuffer(BaseBuffer):
    """
    Rollout buffer used in on-policy algorithms like A2C/PPO.
    It corresponds to ``buffer_size`` transitions collected
    using the current policy.
...
class EvaluationAvecRolloutBuffer(BaseBuffer):
    """
    Rollout buffer used in on-policy algorithms like A2C/PPO.
    It corresponds to ``buffer_size`` transitions collected
    using the current policy.
    ...
    def compute_returns_and_advantage(self, dones: np.ndarray) -> None:
        ...
        for step in reversed(range(self.buffer_size)):
            ...
            self.returns[step] = self.rewards[step] + self.gamma * next_return * next_non_terminal
